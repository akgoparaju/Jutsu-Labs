...
│
├─ Classify Trend:
│  ├─ IF (T_norm > 0.20) AND (SMA_fast > SMA_slow) → BullStrong
│  ├─ IF (T_norm < -0.30) AND (SMA_fast < SMA_slow) → BearStrong
│  └─ ELSE → Sideways
│
├─ Classify Volatility:
│  ├─ IF z_score > 1.0 → High
...
│  │  └─ Replace Cash with: 40% TMF + 60% Cash
│  └─ IF Bond Bear (TLT SMA_fast < SMA_slow):
│     └─ Replace Cash with: 40% TMV + 60% Cash
│
...
│
├─ Check Rebalancing:
│  └─ IF weight deviation > 2.5% → Execute rebalance
│
END
```
