...
class TestDataSyncBasic:
    ...
    def test_initialization(self, in_memory_session):
        """Test DataSync initialization."""
        sync = DataSync(session=in_memory_session)
        assert sync.session == in_memory_session

    def test_sync_symbol_no_existing_data(self, in_memory_session, mock_fetcher, sample_bars):
    ...
    def test_sync_symbol_incremental_update(self, in_memory_session, mock_fetcher, sample_bars):
        """Test incremental update (fetching newer data)."""
        # First sync: store initial data
        sync = DataSync(session=in_memory_session)
        ...
        # Verify incremental fetch: should start from day after last bar (Jan 6)
        last_call_args = mock_fetcher.fetch_bars.call_args[1]
        assert last_call_args['start_date'] == datetime(2024, 1, 6, tzinfo=timezone.utc)
        assert last_call_args['end_date'] == datetime(2024, 1, 10, tzinfo=timezone.utc)
...
class TestDataSyncBackfill:
    ...
    def test_metadata_preserves_most_recent_timestamp(self, in_memory_session, mock_fetcher):
        ...
        metadata_before = sync._get_metadata('AAPL', '1D')
        ...
        metadata_after = sync._get_metadata('AAPL', '1D')
...
class TestDataSyncMetadata:
    ...
    def test_metadata_created_on_first_sync(self, in_memory_session, mock_fetcher, sample_bars):
    ...
    def test_get_sync_status(self, in_memory_session, mock_fetcher, sample_bars):
...
class TestDataSyncValidation:
    ...
    def test_validate_data_all_valid(self, in_memory_session, mock_fetcher, sample_bars):
    ...
    def test_validate_data_detects_invalid_ohlc(self, in_memory_session):
        ...
        in_memory_session.add(invalid_bar)
...
class TestDataSyncAudit:
    ...
    def test_audit_log_created_on_sync(self, in_memory_session, mock_fetcher, sample_bars):
        ...
        logs = sync.get_audit_logs(symbol='AAPL')
        assert len(logs) == 1
        assert logs[0].operation == 'sync'
        assert logs[0].status == 'success'
        assert logs[0].bars_affected == 10

    def test_audit_log_on_error(self, in_memory_session, mock_fetcher):
...
class TestDataSyncForceRefresh:
    ...
    def test_force_refresh_ignores_metadata(self, in_memory_session, mock_fetcher, sample_bars):
