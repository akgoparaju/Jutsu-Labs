"""
Unit tests for MACD_Trend_v3 strategy (zero-line MACD filter variation).

Tests cover:
- Initialization (5 tests)
- Symbol validation (4 tests)
- State determination with zero-line filter (9 tests - includes 3 zero-line edge cases)
...
Key Difference from MACD_Trend:
- Momentum filter: MACD_Line > 0 (zero-line) instead of MACD_Line > Signal_Line
"""
import pytest
from decimal import Decimal
from datetime import datetime, timezone
from unittest.mock import MagicMock, patch
from jutsu_engine.strategies.MACD_Trend_v3 import MACD_Trend_v3
...
@pytest.fixture
def strategy():
    """Create MACD_Trend_v3 strategy with default parameters."""
    return MACD_Trend_v3()


@pytest.fixture
def custom_strategy():
    """Create MACD_Trend_v3 strategy with custom parameters."""
    return MACD_Trend_v3(
        macd_fast_period=10,
        macd_slow_period=20,
...
# ========================================
# 3. State Determination Tests (9 tests)
# Zero-line filter: MACD_Line > 0 instead of MACD_Line > Signal_Line
# ========================================

def test_determine_state_in_all_conditions_met_macd_above_zero(strategy):
    """Test IN state when all 3 conditions met (MACD > 0)."""
    # Price > EMA, MACD > 0 (zero-line), VIX <= 30
    state = strategy._determine_state(
        price=Decimal('405.00'),
        ema=Decimal('400.00'),
        macd_line=Decimal('1.5'),      # Above zero
        signal_line=Decimal('2.0'),    # Signal_Line not used in V3
        vix=Decimal('20.0')
    )
    assert state == 'IN'
...
def test_determine_state_out_trend_fails(strategy):
    ...
    # Price < EMA (trend fails), MACD > 0, VIX <= 30
    state = strategy._determine_state(
        price=Decimal('395.00'),
        ema=Decimal('400.00'),
        macd_line=Decimal('1.5'),      # Above zero
        signal_line=Decimal('0.5'),    # Not relevant
        vix=Decimal('20.0')
    )
    assert state == 'OUT'


def test_determine_state_out_momentum_fails_macd_negative(strategy):
    """Test OUT state when momentum condition fails (MACD < 0)."""
    # Price > EMA, MACD < 0 (below zero-line), VIX <= 30
    state = strategy._determine_state(
        price=Decimal('405.00'),
        ema=Decimal('400.00'),
        macd_line=Decimal('-0.5'),     # Below zero (FAIL)
        signal_line=Decimal('-1.0'),   # Not relevant
        vix=Decimal('20.0')
    )
    assert state == 'OUT'
...
def test_determine_state_out_vix_fails(strategy):
    ...
    # Price > EMA, MACD > 0, VIX > 30 (volatility fails)
    state = strategy._determine_state(
        price=Decimal('405.00'),
        ema=Decimal('400.00'),
        macd_line=Decimal('1.5'),      # Above zero
...
def test_determine_state_out_multiple_failures(strategy):
    ...
    # Price < EMA, MACD < 0, VIX > 30 (all fail)
    state = strategy._determine_state(
        price=Decimal('395.00'),
        ema=Decimal('400.00'),
        macd_line=Decimal('-0.5'),     # Below zero
        signal_line=Decimal('-1.0'),   # Not relevant
        vix=Decimal('35.0')
    )
    assert state == 'OUT'
...
def test_determine_state_boundary_vix_equals_threshold(strategy):
    ...
    # Price > EMA, MACD > 0, VIX = 30.0 (boundary case)
    state = strategy._determine_state(
        price=Decimal('405.00'),
        ema=Decimal('400.00'),
        macd_line=Decimal('1.5'),      # Above zero
...
def test_determine_state_macd_exactly_at_zero(strategy):
    """Test OUT state when MACD exactly at zero (edge case)."""
    # Price > EMA, MACD = 0.0 (exactly at zero-line), VIX <= 30
    # MACD > 0 is FALSE when MACD = 0, so state should be OUT
    state = strategy._determine_state(
        price=Decimal('405.00'),
        ema=Decimal('400.00'),
        macd_line=Decimal('0.0'),      # Exactly at zero (FAIL)
        signal_line=Decimal('0.5'),    # Not relevant
        vix=Decimal('20.0')
    )
    assert state == 'OUT'
...
def test_determine_state_macd_just_above_zero(strategy):
    """Test IN state when MACD just above zero (entry trigger)."""
    # Price > EMA, MACD slightly above 0, VIX <= 30
    state = strategy._determine_state(
        price=Decimal('405.00'),
        ema=Decimal('400.00'),
        macd_line=Decimal('0.01'),     # Just above zero (PASS)
        signal_line=Decimal('-0.5'),   # Not relevant
        vix=Decimal('20.0')
    )
    assert state == 'IN'
...
def test_determine_state_macd_just_below_zero(strategy):
    """Test OUT state when MACD just below zero (exit trigger)."""
    # Price > EMA, MACD slightly below 0, VIX <= 30
    state = strategy._determine_state(
        price=Decimal('405.00'),
        ema=Decimal('400.00'),
        macd_line=Decimal('-0.01'),    # Just below zero (FAIL)
        signal_line=Decimal('0.5'),    # Not relevant
        vix=Decimal('20.0')
    )
    assert state == 'OUT'
...
@patch('jutsu_engine.strategies.MACD_Trend_v3.atr')
...
@patch('jutsu_engine.strategies.MACD_Trend_v3.macd')
@patch('jutsu_engine.strategies.MACD_Trend_v3.ema')
def test_on_bar_state_change_triggers_rebalance(mock_ema, mock_macd, strategy, sample_qqq_bar):
    """Test that on_bar() rebalances on state change (zero-line filter)."""
    import pandas as pd

    # Setup mocks - OUT state (MACD < 0)
    mock_macd.return_value = (
        pd.Series([Decimal('-0.5')]),  # MACD line below zero
        pd.Series([Decimal('-1.0')]),  # Signal line (not used)
...
@patch('jutsu_engine.strategies.MACD_Trend_v3.ema')
@patch('jutsu_engine.strategies.MACD_Trend_v3.atr')
def test_integration_full_in_to_out_cycle_zero_line_filter(mock_atr, mock_ema, mock_macd, strategy):
    """Test full cycle: None → IN → OUT with zero-line filter."""
    import pandas as pd

    # Setup mocks for IN state (MACD > 0, all conditions met)
    mock_macd.return_value = (
        pd.Series([Decimal('1.5')]),   # MACD line > 0
        pd.Series([Decimal('0.5')]),   # Signal line (not used)
    ...
    # Change mocks to OUT state (MACD < 0)
    mock_macd.return_value = (
        pd.Series([Decimal('-0.5')]),  # MACD line < 0 (below zero-line)
        pd.Series([Decimal('-1.0')]),  # Signal line (not used)
        pd.Series([Decimal('0.5')])    # Histogram
    )
...
def test_integration_never_generates_sqqq_signals(strategy):
    ...
    with patch('jutsu_engine.strategies.MACD_Trend_v3.macd') as mock_macd, \
         patch('jutsu_engine.strategies.MACD_Trend_v3.ema') as mock_ema:

        # Setup mocks for OUT state (MACD < 0, bearish)
        mock_macd.return_value = (
            pd.Series([Decimal('-1.5')]),  # MACD line < 0
            pd.Series([Decimal('-1.0')]),  # Signal line (not used)
            pd.Series([Decimal('-0.5')])   # Histogram
