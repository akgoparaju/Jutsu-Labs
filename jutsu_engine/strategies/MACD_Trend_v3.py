"""
MACD-Trend Strategy V3 (V7.0): Conservative zero-line MACD filter variation.

This strategy is identical to MACD_Trend (V5.0) except for ONE KEY DIFFERENCE:
- Uses MACD zero-line crossover instead of MACD/Signal crossover for momentum filter
- More conservative: Requires MACD_Line > 0 (above zero-line) instead of MACD_Line > Signal_Line
...
- V5.0: MACD_Line > Signal_Line (fast, responsive to short-term momentum shifts)
- V7.0: MACD_Line > 0 (slower, more conservative, waits for MACD to cross above zero-line)

More Info: Strategy Specification_ MACD-Trend (V5.0).md (base specification)
"""
...
class MACD_Trend_v3(Strategy):
    """
    MACD-Trend V3 Strategy: Conservative trend-following with zero-line MACD filter.

    Trades TQQQ (3x bull) or CASH based on:
    - Main Trend: 100-day EMA (Price > EMA = Up)
    - Momentum: MACD Line > 0 (zero-line crossover)
    ...
    - MACD_Line > 0 (momentum above zero-line) ← DIFFERENCE FROM V5.0
    - VIX <= 30 (market is calm)

    Exit Conditions (ANY 1 triggers):
    - Price < 100-day EMA (main trend fails)
    - MACD_Line < 0 (momentum below zero-line) ← DIFFERENCE FROM V5.0
    ...
    def __init__(
        ...
        """
        Initialize MACD-Trend V3 strategy.
    ...
    def _validate_required_symbols(self) -> None:
        ...
        if missing_symbols:
            raise ValueError(
                f"MACD_Trend_v3 requires symbols {required_symbols} but "
    ...
    def on_bar(self, bar):
        ...
        Processes:
        - QQQ bars: State detection (MACD zero-line, EMA, VIX evaluation)
        ...
        # Build decision reason (MODIFIED: zero-line check instead of signal crossover)
        trend_status = f"Price({current_price:.2f}) {'>' if current_price > current_ema else '<='} EMA({current_ema:.2f})"
        momentum_status = f"MACD({current_macd_line:.4f}) {'>' if current_macd_line > Decimal('0.0') else '<='} 0"
    ...
    def _determine_state(
        ...
        Args:
            ...
            signal_line: Current MACD signal line value (not used in V3)
        ...
        momentum_is_bullish = macd_line > Decimal('0.0')  # ← CHANGED: zero-line filter
    ...
    def _execute_entry(self, signal_bar):
        ...
        state_desc = "IN: Price > EMA AND MACD > 0 AND VIX calm"
