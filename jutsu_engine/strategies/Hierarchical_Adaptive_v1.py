"""
Hierarchical Adaptive v1.0: Capital-preservation-first strategy with VIX master switch.

This is a 3-filter hierarchical "strategy-of-strategies" that ensures risk is only
taken when market conditions are favorable.

Filter Hierarchy:
1. VIX Master Switch: Primary risk-on/risk-off filter. If VIX > VIX_EMA, go to CASH.
2. Kalman Regime Filter: Identifies market personality using Adaptive Kalman Filter.
3. Adaptive MACD Signal: Regime-specific MACD/EMA parameters for entry triggers.

Core Innovation: Triple-layer risk management prevents trading in unfavorable conditions:
- Filter 1 (VIX) blocks all trading during high volatility
- Filter 2 (Kalman) avoids choppy/neutral markets
- Filter 3 (MACD) validates trend direction within regime

Strategy Logic:
- VIX > VIX_EMA: CASH (master switch overrides all other signals)
...
Signal Sources: QQQ (Kalman + MACD), VIX (volatility filter)
All 28 Parameters: Fully configurable for grid-search and WFO optimization
"""
...
logger = logging.getLogger('STRATEGY.HIERARCHICAL_ADAPTIVE_V1')
...
class Hierarchical_Adaptive_v1(Strategy):
    """
    Hierarchical 3-filter strategy with capital-preservation-first philosophy.

    Uses VIX volatility filter as master switch, then Kalman Filter for regime
    classification, finally applying regime-specific MACD and EMA parameters.
    Trades across 4 vehicles (TQQQ, QQQ, SQQQ, CASH) based on 4 distinct market regimes.

    Performance Targets:
        - Processing Speed: <2ms per bar (including VIX + Kalman + MACD calculations)
    ...
    Example:
        strategy = Hierarchical_Adaptive_v1(
            ...
            macd_slow_sb=26,
            macd_signal_sb=9,
            # ... (all 28 parameters configurable)
        )
    """
    ...
    def __init__(
        self,
        # ==================================================================
        # TIER 0: VIX VOLATILITY FILTER (Master Risk Switch)
        # ==================================================================
        vix_ema_period: int = 20,

        # ==================================================================
        # TIER 1: KALMAN FILTER PARAMETERS (Regime Filter)
        # ==================================================================
        ...
        name: str = "Hierarchical_Adaptive_v1"
    ):
        """
        Initialize Hierarchical Adaptive strategy with 28 configurable parameters.

        Args:
            vix_ema_period: VIX EMA period for volatility filter (default: 20)
            ...
            name: Strategy name (default: 'Hierarchical_Adaptive_v1')

        Raises:
            ValueError: If thresholds not properly ordered, MACD parameters invalid, or vix_ema_period < 1
        """
        super().__init__()
        self.name = name
        self._trade_logger = trade_logger

        # Validate VIX EMA period
        if vix_ema_period < 1:
            raise ValueError(
        ...
        logger.info(
            f"Initialized {name} with 28 parameters: "
            f"VIX[ema={vix_ema_period}], "
    ...
    def init(self):
        ...
        - Stop-loss tracking
        - Maximum lookback period calculation (including VIX EMA)

        Side Effects:
            - Creates Kalman filter instance
            - Initializes current_regime, current_vehicle, leveraged_stop_price
            - Calculates max_lookback from all parameter periods (including VIX)
        """
        ...
        # Calculate maximum lookback period needed for any indicator (including VIX)
        ...
        logger.info(
            f"{self.name} initialized with Kalman filter, VIX filter, and "
            f"max_lookback={self.max_lookback}"
        )

    def on_bar(self, bar: MarketDataEvent):
        """
        Process each bar with hierarchical 3-filter logic:
        1. Check VIX filter (master switch) → CASH if triggered
        2. Update Kalman filter with QQQ data → trend_strength
        3. Determine regime from trend_strength
        4. Check stop-loss for leveraged positions
        5. If regime changed: Execute hierarchical logic for new regime
        6. If target vehicle changed: Rebalance portfolio

        Args:
            bar: New market data bar with OHLCV data

        Processing Flow:
            VIX check → QQQ bar → Kalman update → trend_strength → regime determination
            → regime logic (EMA + MACD checks) → target vehicle → rebalance

        Note:
            Only processes signal_symbol (QQQ) bars. Other symbols (TQQQ, SQQQ, VIX)
        ...
        # Step 2: Check VIX filter FIRST (master switch)
        if self._check_vix_filter():
            # Log VIX filter trigger
            if self._trade_logger:
                ...
                if len(vix_closes) >= self.vix_ema_period:
                    ...
                    current_vix_ema = Decimal(str(vix_ema_series.iloc[-1]))

                    self._trade_logger.log_strategy_context(
                        timestamp=bar.timestamp,
                        symbol=self.vix_symbol,
                        strategy_state="VIX Filter Triggered (Master Switch)",
                        decision_reason=f"VIX {vix_closes.iloc[-1]:.2f} > VIX_EMA({self.vix_ema_period}) {current_vix_ema:.2f} → CASH",
                        indicator_values={
                            'vix': float(vix_closes.iloc[-1]),
                            'vix_ema': float(current_vix_ema)
                        },
                        threshold_values={'vix_ema_period': self.vix_ema_period}
                    )

            # If we have a position, liquidate
            if self.current_vehicle:
                logger.info(
                    f"VIX filter triggered at {bar.timestamp}: liquidating {self.current_vehicle} → CASH"
                )
        ...
        # Step 3: Update Kalman filter with volume-adjusted model
        ...
        # Step 4: Determine regime from trend strength
        new_regime = self._determine_regime(trend_strength)

        # Step 5: Check for stop-loss hit BEFORE regime logic
        ...
        # Step 6: Execute regime logic (even if regime hasn't changed)
        ...
        # Step 7: Rebalance if target vehicle changed
    ...
    def _check_vix_filter(self) -> bool:
        ...
        VIX Filter Logic (from spec):
        - IF VIX > VIX_EMA → CASH (block all trading)
        - Uses on-demand EMA calculation for consistency with regime logic

        Returns:
            True if VIX filter triggered (go to CASH), False otherwise

        Side Effects:
            - Reads VIX closes via get_closes() for EMA calculation
        """
        # Get VIX closes for EMA calculation
        vix_closes = self.get_closes(lookback=self.vix_ema_period, symbol=self.vix_symbol)

        if len(vix_closes) < self.vix_ema_period:
            logger.debug(
                f"Insufficient VIX data for EMA: {len(vix_closes)} < {self.vix_ema_period}"
            )
        ...
        # Check VIX filter condition
        if vix_closes.iloc[-1] > current_vix_ema:
            logger.debug(
                f"VIX filter triggered: VIX {vix_closes.iloc[-1]:.2f} > "
                f"VIX_EMA({self.vix_ema_period}) {current_vix_ema:.2f} → CASH"
            )
            return True
