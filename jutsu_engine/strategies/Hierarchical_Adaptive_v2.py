"""
Hierarchical Adaptive v2.0: Kalman-Driven Exposure Overlay for QQQ / TQQQ

This is a complete paradigm shift from v1 - moving from discrete regime filtering
to continuous exposure scaling.

v2.0 Scope:
- Long-side only (QQQ + TQQQ exposure scaling, no SQQQ hedge)
- Continuous exposure: Always invested (except extreme stress), scale between 0.5x and 1.3x
...
Key Innovation: Continuous exposure throttle vs v1's discrete all-or-nothing positions.
...
4. VIX compression P_VIX (soft filter, not hard gate like v1)
5. Drawdown governor P_DD (linear compression)
...
logger = logging.getLogger('STRATEGY.HIERARCHICAL_ADAPTIVE_V2')


class Hierarchical_Adaptive_v2(Strategy):
    """
    Hierarchical Adaptive v2.0: Kalman-Driven Exposure Overlay for QQQ / TQQQ

    v2 paradigm shift: Continuous exposure scaling (0.5x to 1.3x) vs v1's discrete regime filtering.
    ...
    - Tier 4: Drawdown governor (risk limiter)
    - Tier 5: QQQ/TQQQ position mapping

    v2.0 Scope: Long-side only (QQQ + TQQQ), no SQQQ hedge.
    ...
    def __init__(
        ...
        """
        Initialize Hierarchical Adaptive v2 strategy with 20 configurable parameters.
        ...
        Args:
            ...
            name: Strategy name (default: 'Hierarchical_Adaptive_v2')
        ...
        logger.info(
            ...
            f"DD[soft={DD_soft}, hard={DD_hard}], "
    ...
    def on_bar(self, bar: MarketDataEvent) -> None:
        ...
        6. Apply drawdown governor → E_raw
        ...
        # ===== TIER 6: Apply drawdown governor =====
            ...
            if self._trade_logger:
                ...
                self._trade_logger.log_strategy_context(
                    ...
                    decision_reason=(
                        ...
                        f"DD governor {P_DD:.3f} → Final exposure {E_t:.3f}"
                    ),
                    indicator_values={
    ...
    def _apply_drawdown_governor(
        ...
        """
        Linear compression between DD_soft and DD_hard.
        P_DD = 1.0 if DD < DD_soft
        P_DD = linear interpolation if DD_soft < DD < DD_hard
        P_DD = p_min if DD > DD_hard
