"""
Hierarchical Adaptive v5.1: Commodity-Augmented Regime Allocator with DXY Filter

v5.1 extends v5.0 with Dollar Index (DXY) Momentum Filter:

1. **Enhanced Hedge Preference Signal** (NEW v5.1):
   - Paper Hedge: Corr(QQQ, TLT) < 0.2 AND DXY > SMA_50 (bonds work)
   - Hard Hedge: Corr(QQQ, TLT) >= 0.2 OR DXY < SMA_50 (use GLD/SLV)
   - Addresses currency debasement scenarios where dollar weakness favors gold

2. **Updated 9-Cell Allocation Matrix** (v5.1 changes):
   - C1 (Bull/Low): 100% TQQQ (was 80/20 in v5.0)
   - C2 (Bull/High): 50% TQQQ, 25% GLD, 25% Cash (was 50/20/30)
   - C9 (Vol-Crush): 100% TQQQ (explicit override cell)

3. **All v5.0 Features Retained**:
   - QQQ/TLT correlation filter
   - Precious metals overlay (GLD/SLV)
   - Silver momentum gate
   - Vol-crush override
...
Performance Targets:
    - Processing Speed: <2ms per bar (DXY calc adds minimal overhead)
...
logger = setup_logger('STRATEGY.HIERARCHICAL_ADAPTIVE_V5_1')
...
class Hierarchical_Adaptive_v5_1(Strategy):
    """
    Hierarchical Adaptive v5.1: Commodity-Augmented Regime Allocator with DXY Filter

    Nine-cell allocation system combining hierarchical trend (Kalman + SMA)
    with binarized volatility (rolling Z-score with hysteresis), hedge preference 
    signal (QQQ/TLT correlation + DXY momentum) for Paper vs Hard asset routing.

    Extended Regime Grid (9 cells):
        | Trend      | Vol  | Hedge | Primary       | Defensive/Overlay           |
        |------------|------|-------|---------------|----------------------------|
        | BullStrong | Low  | N/A   | TQQQ 100%     | -                          |
        | BullStrong | High | N/A   | TQQQ 50%      | GLD 25%, Cash 25%          |
    ...
    Key v5.1 Features:
    - DXY Filter: Dollar Index momentum for Paper vs Hard routing
    - Enhanced Hedge Preference: correlation + DXY dual filter
    - Updated cell allocations (C1: 100% TQQQ, C2: 50/25/25)
    - All v5.0 features: Hysteresis, Vol-Crush, Treasury Overlay
    ...
    def __init__(
        ...
        # ==================================================================
        # HEDGE PREFERENCE SIGNAL (2 parameters - v5.0)
        # ==================================================================
        hedge_corr_threshold: Decimal = Decimal("0.20"),
        hedge_corr_lookback: int = 60,

        # ==================================================================
        # DXY FILTER PARAMETERS (2 parameters - NEW v5.1)
        # ==================================================================
        dxy_symbol: str = "UUP",  # Dollar Index ETF (DXY proxy)
        dxy_sma_period: int = 50,

        # ==================================================================
        # COMMODITY/PRECIOUS METALS OVERLAY (6 parameters - v5.0)
        # ==================================================================
        ...
        name: str = "Hierarchical_Adaptive_v5_1"
    ):
        """
        Initialize Hierarchical Adaptive v5.1 strategy.

        v5.1 extends v5.0 with DXY (Dollar Index) momentum filter for enhanced
        hedge preference routing during currency debasement scenarios.
        ...
        Args:
            ...
            # DXY Filter (NEW v5.1)
            dxy_symbol: Dollar Index ETF symbol (default: 'UUP')
            dxy_sma_period: SMA period for DXY trend (default: 50)

            # Commodity/Precious Metals Overlay (v5.0)
        ...
        # Validate v5.1 parameters
        if dxy_sma_period < 10:
            raise ValueError(f"dxy_sma_period must be >= 10, got {dxy_sma_period}")
        ...
        # Store v5.1 parameters (NEW)
        self.dxy_symbol = dxy_symbol
        self.dxy_sma_period = dxy_sma_period
        ...
        self.hedge_preference: Optional[str] = None  # "Paper" or "Hard"
        ...
        self.current_gld_weight: Decimal = Decimal("0")
        self.current_slv_weight: Decimal = Decimal("0")
        self._end_date: Optional = None
        self._data_handler: Optional = None
        self._intraday_price_cache: Dict[Tuple[str, datetime], Decimal] = {}

        logger.info(
            f"Initialized {name} (v5.1 - DXY-FILTERED REGIME): "
            ...
            f"dxy_symbol={dxy_symbol}, dxy_sma_period={dxy_sma_period}, "
    ...
    def init(self) -> None:
        ...
        self._last_qqq_tlt_corr = None
        self._last_gold_momentum = None
        self._last_silver_relative_strength = None
        self._last_dxy_value = None  # NEW v5.1
        self._last_dxy_sma = None    # NEW v5.1
        self._last_dxy_trend = None  # NEW v5.1

        logger.info(f"Initialized {self.name} (v5.1) with hysteresis state: {self.vol_state}")

    def get_required_warmup_bars(self) -> int:
        """
        Calculate warmup bars needed for v5.1 indicators.

        v5.1 adds DXY filter which may require additional warmup:
        - dxy_sma_period for DXY trend (default 50)
        ...
        # v5.1 indicators (NEW)
        dxy_lookback = self.dxy_sma_period + 10
    ...
    # ===== v5.1 NEW DXY FILTER METHOD =====

    def _calculate_dxy_trend(self, current_bar: MarketDataEvent) -> str:
        """
        Calculate DXY (Dollar Index) trend based on SMA crossover.

        v5.1 Logic:
        - DXY > SMA(dxy_sma_period) → "Bull" (dollar strong, paper hedge OK)
        - DXY <= SMA(dxy_sma_period) → "Bear" (dollar weak, use hard hedge)

        Returns:
            "Bull" or "Bear"
        """
        try:
            dxy_closes = self._get_closes_for_indicator_calculation(
                lookback=self.dxy_sma_period + 10,
                symbol=self.dxy_symbol,
                current_bar=current_bar
            )

            if len(dxy_closes) < self.dxy_sma_period:
                logger.warning("Insufficient DXY data for trend, defaulting to Bull")
                return "Bull"

            dxy_sma = dxy_closes.rolling(window=self.dxy_sma_period).mean().iloc[-1]
            current_dxy = dxy_closes.iloc[-1]

            if pd.isna(dxy_sma):
                return "Bull"

            # Store for debugging/dashboard
            self._last_dxy_value = Decimal(str(current_dxy))
            self._last_dxy_sma = Decimal(str(dxy_sma))

            if current_dxy > dxy_sma:
                self._last_dxy_trend = "Bull"
                logger.debug(f"DXY Trend: BULL (DXY={current_dxy:.2f} > SMA={dxy_sma:.2f})")
                return "Bull"
            else:
                self._last_dxy_trend = "Bear"
                logger.debug(f"DXY Trend: BEAR (DXY={current_dxy:.2f} <= SMA={dxy_sma:.2f})")
                return "Bear"

        except Exception as e:
            logger.warning(f"DXY trend calculation failed: {e}, defaulting to Bull")
            return "Bull"

    # ===== v5.1 MODIFIED HEDGE PREFERENCE METHOD =====

    def _calculate_hedge_preference(self, current_bar: MarketDataEvent) -> str:
        """
        Calculate hedge preference based on QQQ/TLT correlation AND DXY filter.

        v5.1 Logic (expanded from v5.0):
        - PAPER: Corr(QQQ, TLT) < hedge_corr_threshold AND DXY > SMA_50
        - HARD: Corr(QQQ, TLT) >= hedge_corr_threshold OR DXY < SMA_50

        The idea: 
        1. When stocks and bonds move together (positive correlation),
           bonds fail as a hedge (2022 inflationary environment). Switch to gold.
        2. When dollar is weakening (DXY < SMA), gold outperforms as inflation hedge.
           Even if correlation is low, prefer hard assets during currency debasement.

        Returns:
            "Paper" or "Hard"
        """
        try:
            # Step 1: Calculate QQQ/TLT correlation
            ...
            if len(qqq_closes) < self.hedge_corr_lookback or len(tlt_closes) < self.hedge_corr_lookback:
                logger.warning("Insufficient data for correlation, checking DXY only")
                correlation = Decimal("0")  # Default to neutral correlation
            else:
                ...
                if len(qqq_returns) < 10 or len(tlt_returns) < 10:
                    ...
                    if len(common_idx) < 10:
                        ...
                        corr_val = qqq_aligned.corr(tlt_aligned)
                        if pd.isna(corr_val):
                            correlation = Decimal("0")
                        else:
                            correlation = Decimal(str(corr_val))

            self._last_qqq_tlt_corr = correlation

            # Step 2: Calculate DXY trend (NEW v5.1)
            dxy_trend = self._calculate_dxy_trend(current_bar)

            # Step 3: Apply v5.1 logic
            # PAPER: correlation < threshold AND DXY > SMA (dollar strong)
            # HARD: correlation >= threshold OR DXY < SMA (dollar weak)
            
            correlation_triggers_hard = correlation >= self.hedge_corr_threshold
            dxy_triggers_hard = dxy_trend == "Bear"  # DXY < SMA means dollar weak

            if correlation_triggers_hard or dxy_triggers_hard:
                reason = []
                if correlation_triggers_hard:
                    reason.append(f"corr={correlation:.3f} >= {self.hedge_corr_threshold}")
                if dxy_triggers_hard:
                    reason.append(f"DXY={self._last_dxy_value:.2f} < SMA={self._last_dxy_sma:.2f}")
                logger.debug(f"Hedge Preference: HARD ({', '.join(reason)})")
                return "Hard"
            else:
                logger.debug(
                    f"Hedge Preference: PAPER (corr={correlation:.3f} < {self.hedge_corr_threshold} "
                    f"AND DXY={self._last_dxy_value:.2f} > SMA={self._last_dxy_sma:.2f})"
                )
                return "Paper"

        except Exception as e:
            logger.warning(f"Hedge preference calculation failed: {e}, defaulting to Paper")
            return "Paper"
    ...
    # ===== v3.5b INHERITED METHODS (unchanged) =====
    ...
    def _get_cell_id(self, trend_state: str, vol_state: str) -> int:
        ...
        Note: v5.1 uses same 6-cell base as v3.5b, but cells 4 and 6
        have sub-routing based on hedge_preference (Paper vs Hard).
        Cell 9 is reserved for vol-crush override.

        Cell Mapping:
            1: BullStrong + Low
            2: BullStrong + High
            3: Sideways + Low
            4: Sideways + High (sub-routes to Paper/Hard based on hedge)
            5: BearStrong + Low
            6: BearStrong + High (sub-routes to Paper/Hard based on hedge)
            9: Vol-Crush Override (special case)
        """
    ...
    def _get_cell_allocation_v51(
        self,
        cell_id: int,
        hedge_preference: str,
        current_bar: MarketDataEvent,
        vol_crush_triggered: bool = False
    ) -> Tuple[Decimal, Decimal, Decimal, Decimal, Decimal, Decimal, Decimal, Decimal]:
        """
        Get v5.1 allocation weights for cell ID with hedge preference routing.

        v5.1 9-Cell Allocation Matrix (updated from v5.0):
            Cell 1 (Bull/Low):   100% TQQQ (was 80/20 in v5.0)
            Cell 2 (Bull/High):  50% TQQQ, 25% GLD, 25% Cash (was 50/20/30)
            ...
            Cell 9 (Vol-Crush):  100% TQQQ (override)
        ...
        # Cell 9: Vol-Crush Override (NEW explicit in v5.1)
        if vol_crush_triggered and cell_id in [5, 6]:
            # Override bear cells with aggressive recovery position
            w_TQQQ = Decimal("1.0")
            logger.info("Cell 9 (Vol-Crush Override): 100% TQQQ")
            return (w_TQQQ, w_QQQ, w_PSQ, w_TMF, w_TMV, w_GLD, w_SLV, w_cash)

        if cell_id == 1:
            # Cell 1 (Bull/Low): Full Momentum - v5.1 updated to 100% TQQQ
            w_TQQQ = Decimal("1.0")

        elif cell_id == 2:
            # Cell 2 (Bull/High): Cautious Growth - v5.1 updated to 50/25/25
            w_TQQQ = Decimal("0.5")
            w_GLD = Decimal("0.25")
            w_cash = Decimal("0.25")

        elif cell_id == 3:
            # Cell 3 (Sideways/Low): Permanent portfolio mix
            w_QQQ = Decimal("0.6")
            w_GLD = Decimal("0.4")

        elif cell_id == 4:
            # Cell 4 (Sideways/High): Route based on hedge preference
            w_PSQ = Decimal("0.2")
            defensive_weight = Decimal("0.8")

            if hedge_preference == "Paper":
                # Cell 4a: Paper hedge - Use TMF
                ...
                # Cell 4b: Hard hedge - Use GLD/SLV (60/20 per spec)
                w_GLD = Decimal("0.6")
                w_SLV = Decimal("0.2")

        elif cell_id == 5:
            # Cell 5 (Bear/Low): Orderly decline with PSQ
            ...
            # Cell 6 (Bear/High): Crisis regime - route based on hedge preference
            if hedge_preference == "Paper":
                # Cell 6a: Paper hedge - Maximum safety in crash
                w_cash = Decimal("1.0")
            else:
                # Cell 6b: Hard hedge - Black Swan (70% GLD, 30% SLV per spec)
                w_GLD = Decimal("0.7")
                w_SLV = Decimal("0.3")
    ...
    def on_bar(self, bar: MarketDataEvent) -> None:
        """
        Process each bar through v5.1 DXY-filtered regime allocator.
        ...
        7. Calculate Hedge Preference (Paper/Hard) based on QQQ/TLT correlation + DXY
        ...
        # 7. Calculate Hedge Preference (v5.1 - with DXY filter)
        hedge_preference = self._calculate_hedge_preference(bar)
        gold_momentum = self._calculate_gold_momentum(bar)

        # 8. Get cell ID and allocation (v5.1 version)
        cell_id = self._get_cell_id(trend_state, self.vol_state)
        (w_TQQQ, w_QQQ, w_PSQ, w_TMF, w_TMV, w_GLD, w_SLV, w_cash) = \
            self._get_cell_allocation_v51(cell_id, hedge_preference, bar, vol_crush_triggered)

        # Update cell_id for vol-crush override logging
        if vol_crush_triggered and cell_id in [5, 6]:
            cell_id = 9
        ...
        needs_rebalance = self._check_rebalancing_threshold_v51(
        ...
        dxy_log = ""
        if self._last_dxy_value is not None:
            dxy_log = f", DXY={self._last_dxy_value:.2f}/{self._last_dxy_sma:.2f}"

        logger.info(
            f"[{bar.timestamp}] v5.1 Regime | "
            ...
            f"HedgePref={hedge_preference}, GoldMom={gold_momentum}{dxy_log} | "
            ...
            if self._trade_logger:
                for symbol in [
                    ...
                    self._trade_logger.log_strategy_context(
                        ...
                        symbol=symbol,
                        strategy_state=f"v5.1 Cell {cell_id}: {trend_state}/{self.vol_state}/{hedge_preference}",
                        decision_reason=(
                            f"T_norm={T_norm:.3f}, z={z_score:.3f}, "
                            f"hedge_pref={hedge_preference}, vol_crush={vol_crush_triggered}, "
                            f"dxy_trend={self._last_dxy_trend}"
                        ),
                        indicator_values={
                            'T_norm': float(T_norm),
                            ...
                            'dxy_value': float(self._last_dxy_value) if self._last_dxy_value else 0.0,
                            'dxy_sma': float(self._last_dxy_sma) if self._last_dxy_sma else 0.0
                        },
                        threshold_values={
                            ...
                            'dxy_sma_period': self.dxy_sma_period
                        }
                    )

            self._execute_rebalance_v51(w_TQQQ, w_QQQ, w_PSQ, w_TMF, w_TMV, w_GLD, w_SLV)

    def _check_rebalancing_threshold_v51(
        self,
        ...
        """Check if portfolio weights drifted beyond threshold (v5.1 version)."""
        weight_deviation = (
    ...
    def _execute_rebalance_v51(
        ...
        """
        Rebalance portfolio to target weights using two-phase execution (v5.1 version).
        """
