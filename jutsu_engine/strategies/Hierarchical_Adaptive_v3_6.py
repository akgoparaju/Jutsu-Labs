...
Changes FROM 3.5b:
****
Action 1: "Cash is King" in Cell 4
In the "Chop" zone (Sideways + High Vol), the most efficient position is 100% Cash. By forcing this, you effectively eliminate the -6.72% drag, which will immediately boost your total CAGR.
Change: Disable the Treasury Overlay for Cell 4.
New Allocation: 100% Cash

Action 2: Stabilize Cell 2 (Bull / High Vol)
You have two options here depending on your goal:
Option A (Smooth Drawdowns): Change allocation to 50% QQQ / 50% Cash. This acknowledges that "High Vol" is risky and reduces your exposure. This increases Sharpe Ratio but might not catch the full 33% upside.
"""
...
class Hierarchical_Adaptive_v3_5b(Strategy):
    ...
    def _get_cell_allocation(self, cell_id: int) -> tuple[Decimal, Decimal, Decimal, Decimal]:
        ...
        6-Cell Allocation Matrix (Base Weights):
            ...
            Cell 2 (Bull/High):  0% TQQQ, 50% QQQ, 50% Cash (Net Beta 0.5)
